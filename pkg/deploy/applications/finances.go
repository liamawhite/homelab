...
const (
	financesImageName = "ghcr.io/liamawhite/finances"
	financesPort      = 8080

	// financesUID is the non-root UID the container's binary runs as
	// (matches applications/finances/Dockerfile's scratch stage - no
	// /etc/passwd entry exists there either, same convention as
	// workouts/shopping/trips). The PVC's fsGroup below has to match it:
	// Longhorn (like any dynamically provisioned volume) mounts owned by
	// root by default, which a non-root container can't write to without
	// fsGroup chowning the mount to this GID first.
	financesUID = 65534
)

// BuildFinancesImage builds and pushes the finances app's image (Go backend
// with the built React frontend go:embedded into it - see
// applications/finances/Dockerfile) via buildx/BuildKit through the Pulumi
...
// (applications/finances), not the repo root - repoRoot is still resolved
...
func BuildFinancesImage(ctx *pulumi.Context, name, ghcrUsername, ghcrToken string, opts ...pulumi.ResourceOption) (*dockerbuild.Image, error) {
	...
	return dockerbuild.NewImage(ctx, name, &dockerbuild.ImageArgs{
		Context: &dockerbuild.BuildContextArgs{
			Location: pulumi.String(filepath.Join(repoRoot, "applications/finances")),
		},
		Dockerfile: &dockerbuild.DockerfileArgs{
			Location: pulumi.String(filepath.Join(repoRoot, "applications/finances/Dockerfile")),
		},
		...
		Tags: pulumi.StringArray{
			pulumi.String(financesImageName + ":latest"),
			pulumi.Sprintf("%s:%s", financesImageName, buildTag),
		},
	}, opts...)
}

// Finances represents the household finances app: a single Go binary
// (Connect API + embedded React frontend) backed by SQLite on a Longhorn
// PVC, reachable only over Tailscale - same exposure pattern as
// Workouts/Shopping/Trips.
...
func (f *Finances) TailscaleRedirect() ingress.RedirectRoute {
	return f.redirect
}

// FinancesArgs contains the configuration for Finances.
type FinancesArgs struct {
	// Namespace is created centrally by pkg/deploy/namespaces.go
	// (FinancesNamespace) and passed in here - this component does not
	// create it.
	...
	// GHCRUsername/GHCRToken authenticate BuildFinancesImage's push.
	GHCRUsername string
	GHCRToken    string
}

// NewFinances builds the finances image, provisions its PVC, and deploys it
// behind a Tailscale-only waypoint - same shape as NewWorkouts/NewShopping,
...
// why).
func NewFinances(ctx *pulumi.Context, name string, args *FinancesArgs, opts ...pulumi.ResourceOption) (*Finances, error) {
	finances := &Finances{}

	err := ctx.RegisterComponentResource("homelab:applications:finances", name, finances, opts...)
	if err != nil {
		return nil, err
	}

	localOpts := append(opts, pulumi.Parent(finances))

	labels := pulumi.StringMap{"app": pulumi.String(name)}

	image, err := BuildFinancesImage(ctx, fmt.Sprintf("%s-image", name), args.GHCRUsername, args.GHCRToken, localOpts...)
	if err != nil {
		return nil, err
	}
		...
		Spec: &appsv1.DeploymentSpecArgs{
			...
			Template: &corev1.PodTemplateSpecArgs{
				...
				Spec: &corev1.PodSpecArgs{
					...
					// non-root container (see financesUID's doc comment)
					// can't write its SQLite file at all.
					SecurityContext: &corev1.PodSecurityContextArgs{
						FsGroup: pulumi.Int(financesUID),
					},
					Containers: corev1.ContainerArray{
						&corev1.ContainerArgs{
							...
							Env: corev1.EnvVarArray{
								&corev1.EnvVarArgs{Name: pulumi.String("DB_PATH"), Value: pulumi.String("/data/finances.db")},
							},
							Ports: corev1.ContainerPortArray{
								&corev1.ContainerPortArgs{ContainerPort: pulumi.Int(financesPort)},
							},
		...
		Spec: &corev1.ServiceSpecArgs{
			...
			Ports: corev1.ServicePortArray{
				&corev1.ServicePortArgs{
					Port:       pulumi.Int(80),
					TargetPort: pulumi.Int(financesPort),
				},
			},
		},
	...
	finances.Namespace = args.Namespace.ToStringOutput()
	finances.ServiceName = service.Metadata.Name().Elem()
	finances.redirect = tsIngress.Redirect

	if err := ctx.RegisterResourceOutputs(finances, pulumi.Map{
		"namespace":   finances.Namespace,
		"serviceName": finances.ServiceName,
	}); err != nil {
		return nil, err
	}
