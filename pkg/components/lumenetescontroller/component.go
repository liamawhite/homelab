// Package lumenetescontroller deploys the lumenetes-controller (see
// applications/lumenetes): a Deployment that syncs live Philips
// Hue light status into Light custom resources (see pkg/crds/lumenetes).
//
...
const serviceAccountName = "lumenetes-controller"

// LumenetesController represents the deployed lumenetes-controller.
type LumenetesController struct {
...
type LumenetesControllerArgs struct {
...
func NewLumenetesController(ctx *pulumi.Context, name string, args *LumenetesControllerArgs, opts ...pulumi.ResourceOption) (*LumenetesController, error) {
	lc := &LumenetesController{}
	err := ctx.RegisterComponentResource("homelab:kubernetes:lumenetes-controller", name, lc, opts...)
	if err != nil {
		return nil, err
	}
	...
	leaseRole, err := rbacv1.NewRole(ctx, fmt.Sprintf("%s-lease-role", name), &rbacv1.RoleArgs{
		Metadata: &metav1.ObjectMetaArgs{
			Name:      pulumi.String("lumenetes-controller-leases"),
			Namespace: args.Namespace,
		},
		Rules: rbacv1.PolicyRuleArray{
			&rbacv1.PolicyRuleArgs{
	...
	// (see applications/lumenetes.go) - Command picks which of its two binaries
				...
				Spec: &corev1.PodSpecArgs{
					...
					Containers: corev1.ContainerArray{
						&corev1.ContainerArgs{
							Name:    pulumi.String("lumenetes-controller"),
							Image:   args.Image,
							Command: pulumi.StringArray{pulumi.String("/lumenetes-controller")},
