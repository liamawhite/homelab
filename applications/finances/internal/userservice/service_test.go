...
func TestService_CreateUser_ThenListUsers(t *testing.T) {
	...
	created, err := svc.CreateUser(ctx, connect.NewRequest(&v1.CreateUserRequest{Name: "Guest"}))
	if err != nil {
	...
	if created.Msg.GetUser().GetName() != "Guest" {
		t.Fatalf("CreateUser() name = %q, want %q", created.Msg.GetUser().GetName(), "Guest")
	}
	...
	if len(listed.Msg.GetUsers()) != 3 {
		t.Fatalf("ListUsers() = %d users, want 3 (2 seeded + 1 created)", len(listed.Msg.GetUsers()))
	}
}
...
func TestService_DeleteUser_ThenListUsers(t *testing.T) {
	...
	if len(listed.Msg.GetUsers()) != 2 {
		t.Fatalf("ListUsers() after delete = %d users, want 2 (seeded only)", len(listed.Msg.GetUsers()))
	}
}
