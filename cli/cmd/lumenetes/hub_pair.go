...
infra.yaml (lumenetes.hue.bridges) so other lights commands can use it.
...
Example:
  homelab lumenetes hub pair
  homelab lumenetes hub pair --ip 192.168.0.165`,
	RunE: runHubPair,
}
...
func runHubPair(cmd *cobra.Command, args []string) error {
	...
	if ip == "" {
		...
		case 1:
			ip = bridges[0].IP
		default:
			return fmt.Errorf("found %d bridges, pass --ip to pick one (run 'homelab lumenetes hub ls' to see them)", len(bridges))
		}
	}
