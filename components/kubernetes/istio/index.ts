...
export class Istio extends pulumi.ComponentResource {
    ...
    constructor(name: string, args: IstioArgs, opts: pulumi.ComponentResourceOptions & { provider: k8s.Provider }) {
        ...
        const repositoryOpts = { repo: "https://istio-release.storage.googleapis.com/charts" }

        const crds = new k8s.helm.v4.Chart(`${name}-crds`, {
        ...
        const gatewayApi = new k8s.yaml.ConfigFile(`${name}-gateway-api`, {
            file: GATEWAY_API_URL,
        }, localOpts)

        const istiod = new k8s.helm.v4.Chart(`${name}-istiod`, {
            namespace: namespace.metadata.name,
            chart: "istiod",
            version: versions.istio,
            repositoryOpts,
        ...
        const cni = new k8s.helm.v4.Chart(`${name}-cni`, {
            namespace: namespace.metadata.name,
            chart: "cni",
            version: versions.istio,
            repositoryOpts,
        ...
        const ztunnel = new k8s.helm.v4.Chart(`${name}-ztunnel`, {
            namespace: namespace.metadata.name,
            chart: "ztunnel",
            version: versions.istio,
            repositoryOpts,
